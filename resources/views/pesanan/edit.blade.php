...
<div class="max-w-4xl mx-auto">
    <div class="bg-white overflow-hidden shadow-sm rounded-lg">
        <div class="p-6 bg-white border-b border-gray-200">
            <h2 class="text-2xl font-bold text-gray-900 mb-6">Edit Pesanan #{{ $pesanan->id_pesanan }}</h2>
            ...
            <form method="POST" action="{{ route('pesanan.update', $pesanan->id_pesanan) }}">
                    ...
                    <div class="space-y-6">
                            ...
                            <select name="id_pelanggan" 
                                ...
                                @foreach($pelanggan as $customer)
                                    <option value="{{ $customer->id_pelanggan }}" 
                                            {{ old('id_pelanggan', $pesanan->id_pelanggan) == $customer->id_pelanggan ? 'selected' : '' }}>
                        ...
                        <div>
                            ...
                            <select name="jenis_layanan" 
                                ...
                                <option value="Cuci Kering" {{ old('jenis_layanan', $pesanan->jenis_layanan) == 'Cuci Kering' ? 'selected' : '' }}>Cuci Kering</option>
                                <option value="Cuci Setrika" {{ old('jenis_layanan', $pesanan->jenis_layanan) == 'Cuci Setrika' ? 'selected' : '' }}>Cuci Setrika</option>
                                <option value="Setrika Saja" {{ old('jenis_layanan', $pesanan->jenis_layanan) == 'Setrika Saja' ? 'selected' : '' }}>Setrika Saja</option>
                                <option value="Dry Clean" {{ old('jenis_layanan', $pesanan->jenis_layanan) == 'Dry Clean' ? 'selected' : '' }}>Dry Clean</option>
                            </select>
                            @error('jenis_layanan')
                                <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        ...
                        <div>
                            ...
                            <input type="number" 
                                   name="jumlah_pakaian" 
                                   id="jumlah_pakaian" 
                                   value="{{ old('jumlah_pakaian', $pesanan->jumlah_pakaian) }}"
                                   min="1"
                                   class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm"
                                   required>
                    ...
                    <div class="space-y-6">
                        ...
                        <div>
                            ...
                            <input type="number" 
                                   name="harga_per_kg" 
                                   id="harga_per_kg" 
                                   value="{{ old('harga_per_kg', $pesanan->harga_per_kg) }}"
                                   min="0"
                                   class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm"
                                   required>
                            @error('harga_per_kg')
                        ...
                        <div>
                            ...
                            <input type="datetime-local" 
                                   ...
                                   id="tanggal_selesai" 
                                   value="{{ old('tanggal_selesai', $pesanan->tanggal_selesai ? $pesanan->tanggal_selesai->format('Y-m-d\TH:i') : '') }}"
                                   class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm">
                        ...
                        <div>
                            ...
                            <select name="status" 
                                ...
                                <option value="dalam_proses" {{ old('status', $pesanan->status) == 'dalam_proses' ? 'selected' : '' }}>Dalam Proses</option>
                ...
                <div class="mt-6">
                    ...
                    <textarea name="catatan" 
                              ...
                              rows="3"
                              class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm"
                              placeholder="Catatan khusus untuk pesanan ini...">{{ old('catatan', $pesanan->catatan) }}</textarea>
                    @error('catatan')
                        <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
